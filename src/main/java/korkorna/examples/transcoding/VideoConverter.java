...
public class VideoConverter extends AMediaCoderMixin implements IMediaWriter {
	...
	public VideoConverter(String transcodeFile, IMediaReader reader, OutputFormat outputFormat) {
		this(transcodeFile, reader);
	 ...
	 * data about the container and it's streams. The {@link IMediaReader} must
	 * be configured such that streams will not be dynamically added to the
	 * container, which is the default for {@link IMediaReader}.
	 * 
	 * @param url
	 *            the url or filename of the media destination
	 * @param reader
	 *            the media source
	 * 
	 * @throws IllegalArgumentException
	 *             if the specifed {@link IMediaReader} is configure to allow
	 *             dynamic adding of streams.
	 */

	VideoConverter(String url, IMediaReader reader) {
		...
		if (reader.canAddDynamicStreams())
			throw new IllegalArgumentException(
					"inputContainer is improperly configured to allow " + "dynamic adding of streams.");
	}

	/**
	 * Use a specified {@link IContainer} as a source for and meta data about
	 * the container and it's streams. The {@link IContainer} must be configured
	 * such that streams will not be dynamically added to the container.
	 * 
	 * @param url
	 *            the url or filename of the media destination
	 * @param inputContainer
	 *            the source media container
	 * 
	 * @throws IllegalArgumentException
	 *             if the specifed {@link IContainer} is not a of type READ or
	 *             is configure to allow dynamic adding of streams.
	 */

	VideoConverter(String url, IContainer inputContainer) {
		...
		mContainerFormat.setOutputFormat(mInputContainer.getContainerFormat().getInputFormatShortName(), getUrl(),
				null);
	}

	/**
	 * Create a MediaWriter which will require subsequent calls to
	 * {@link #addVideoStream} and/or {@link #addAudioStream} to configure the
	 * writer. Streams may be added or further configured as needed until the
	 * first attempt to write data.
	 * 
	 * <p>
	 *
	 * To write data call to {@link #onAudioSamples} and/or
	 * {@link #onVideoPicture}.
	 *
	 * </p>
	 *
	...
	public int addAudioStream(int inputIndex, int streamId, int channelCount, int sampleRate) {
		...
		try {
			return addAudioStream(inputIndex, streamId, codec, channelCount, sampleRate);
		} finally {
			if (codec != null)
				codec.delete();
		}
	...
	/**
	 * Add a audio stream. The time base defaults to {@link #DEFAULT_TIMEBASE}
	 * and the audio format defaults to {@link #DEFAULT_SAMPLE_FORMAT}. The new
	 * {@link IStream} is returned to provide an easy way to further configure
	 * the stream.
	 * 
	 * @param inputIndex
	 *            the index that will be passed to {@link #onAudioSamples} for
	 *            this stream
	 * @param streamId
	 *            a format-dependent id for this stream
	 * @param codec
	 *            the codec to used to encode data, to establish the codec see
	 *            {@link com.xuggle.xuggler.ICodec}
	 * @param channelCount
	 *            the number of audio channels for the stream
	 * @param sampleRate
	 *            sample rate in Hz (samples per seconds), common values are
	 *            44100, 22050, 11025, etc.
	 *
	 * @throws IllegalArgumentException
	 *             if inputIndex < 0, the stream id < 0, the codec is NULL or if
	...
	public int addVideoStream(int inputIndex, int streamId, int width, int height) {
		return addVideoStream(inputIndex, streamId, (IRational) null, width, height);
	}
	...
	public int addVideoStream(int inputIndex, int streamId, IRational frameRate, int width, int height) {
		...
		try {
			return addVideoStream(inputIndex, streamId, codec, frameRate, width, height);
		} finally {
			if (codec != null)
				codec.delete();
		}
	...
	public int addVideoStream(int inputIndex, int streamId, ICodec.ID codecId, int width, int height) {
		return addVideoStream(inputIndex, streamId, codecId, null, width, height);
	}

	public int addVideoStream(int inputIndex, int streamId, ICodec.ID codecId, IRational frameRate, int width,
			int height) {
		if (codecId == null)
			throw new IllegalArgumentException("null codecId");
		ICodec codec = ICodec.findEncodingCodec(codecId);
	...
	public int addVideoStream(int inputIndex, int streamId, ICodec codec, int width, int height) {
		return addVideoStream(inputIndex, streamId, codec, null, width, height);
	}

	/**
	 * Add a video stream. The time base defaults to {@link #DEFAULT_TIMEBASE}
	 * and the pixel format defaults to {@link #DEFAULT_PIXEL_TYPE}. The new
	 * {@link IStream} is returned to provide an easy way to further configure
	 * the stream.
	 * 
	 * @param inputIndex
	 *            the index that will be passed to {@link #onVideoPicture} for
	 *            this stream
	 * @param streamId
	 *            a format-dependent id for this stream
	 * @param codec
	...
	public int addVideoStream(int inputIndex, int streamId, ICodec codec, IRational frameRate, int width, int height) {
		...
		try {
			...
			if (supportedFrameRates != null && supportedFrameRates.size() > 0) {
				...
				// throw an error.
				if (IRational.positive(frameRate)
						&& (highestResolution == null || highestResolution.compareTo(frameRate) != 0))
					throw new UnsupportedOperationException(
							"container does not" + " support encoding at given frame rate: " + frameRate);

				// if we got through the supported list and found NO valid
				// resolution, fail.
				if (highestResolution == null)
					throw new UnsupportedOperationException(
							"could not find supported frame rate for container: " + getUrl());
				if (timeBase == null)
			...
			if (timeBase == null) {
				...
				// is too aggressive...
				if (codec.getID() == ICodec.ID.CODEC_ID_MPEG4 && timeBase.getDenominator() > ((1 << 16) - 1)) {
	 ...
	 *            a format-dependent id for this stream
	 * @param codec
	 *            the codec to used to encode data
	 *
	 * @throws IllegalArgumentException
	 *             if inputIndex < 0, the stream id < 0, the codec is NULL or if
	 *             the container is already open.
	...
	private IStream establishStream(int inputIndex, int streamId, ICodec codec) {
		...
		if (stream == null)
			throw new RuntimeException(
					"Unable to create stream id " + streamId + ", index " + inputIndex + ", codec " + codec);
	...
	/**
	 * Set late stream exception policy. When onVideoPicture or onAudioSamples
	 * is passed an unrecognized stream index after the the header has been
	 * written, either an exception is raised, or the media data is silently
	 * ignored. By default exceptions are raised, not masked.
	 *
	 * @param maskLateStreamExceptions
	 *            true if late med
	 * 
	 ...
	 * header has been written, either an exception is raised, or the media data
	 * is silently ignored. By default exceptions are raised, not masked.
	 *
	 ...
	 * <p>
	 *
	 * If true MediaWriter will ask Xuggler to place media data in time stamp
	 * order, which is required for streaming media.
	 *
	 * <p>
	 *
	 * @param forceInterleave
	 *            true if the MediaWriter should force interleaving of media
	 *            data
	 *
	...
	/**
	 * Test if the MediaWriter will forcibly interleave media data. The default
	 * value for this value is true.
	 *
	 ...
	 *
	 * @param inputStreamIndex
	 *            the input stream index value
	 *
	...
	private void encodeVideo(int streamIndex, IVideoPicture picture, BufferedImage image) {
		...
		if (CODEC_TYPE_VIDEO != mStreams.get(outputIndex).getStreamCoder().getCodecType()) {
		...
		super.onVideoPicture(new VideoPictureEvent(this, picture, image, picture.getTimeStamp(), TimeUnit.MICROSECONDS,
				streamIndex));
	...
	public void encodeVideo(int streamIndex, BufferedImage image, long timeStamp, TimeUnit timeUnit) {
		...
		IVideoPicture picture = convertToPicture(streamIndex, image, MICROSECONDS.convert(timeStamp, timeUnit));
	...
	public void encodeAudio(int streamIndex, IAudioSamples samples) {
		...
		try {
			...
			for (int consumed = 0; consumed < samples
					.getNumSamples(); /* in loop */) {
				// encode audio
	...
	public void encodeAudio(int streamIndex, short[] samples, long timeStamp, TimeUnit timeUnit) {
		...
		try {
			...
			/**
			 * We allow people to pass in a null timeUnit for audio as a signal
			 * that time stamps are unknown. This is a common case for audio
			 * data, and Xuggler should handle it if we set a invalid time stamp
			 * on the audio.
			 */
			...
			audioFrame.setComplete(true, sampleCount, coder.getSampleRate(), coder.getChannels(),
					coder.getSampleFormat(), timeStampMicro);
	 ...
	 * 
	 * @param stream
	 *            to destination stream of the image
	 */
	...
	private IVideoPicture convertToPicture(int streamIndex, BufferedImage image, long timeStamp) {
		...
		if (videoConverter == null) {
			...
			IStreamCoder coder = stream.getStreamCoder();
			videoConverter = ConverterFactory.createConverter(ConverterFactory.findDescriptor(image),
					coder.getPixelType(), coder.getWidth(), coder.getHeight(), image.getWidth(), image.getHeight());
	 ...
	 * container. If this has been seen before, it is assumed to be a new stream
	 * and construct the correct coder for it.
	 *
	 * @param inputStreamIndex
	 *            the input index of the stream for which to find the coder
	 * 
	 * @return the coder which will be used to encode data for the specified
	 *         stream
	 */
	...
	private IStream getStream(int inputStreamIndex) {
		...
		if (!getContainer().isHeaderWritten()) {
			...
			if (0 != rv)
				throw new RuntimeException(
						"Error " + IError.make(rv) + ", failed to write header to container " + getContainer()
								+ " while establishing stream " + mStreams.get(getOutputStreamIndex(inputStreamIndex)));
	...
	/**
	 * Test if the {@link MediaWriter} can write streams of this
	 * {@link ICodec.Type}
	 * 
	 * @param type
	 *            the type of codec to be tested
	 *
	 ...
	 *
	 * @param inputStreamIndex
	 *            the index of the stream on the input container
	 * 
	...
	private boolean addStreamFromContainer(int inputStreamIndex) {
		...
		case CODEC_TYPE_AUDIO:
			addAudioStream(inputStream.getIndex(), inputStream.getId(), format.establishOutputCodecId(inputID),
					inputCoder.getChannels(), inputCoder.getSampleRate());
			break;
		case CODEC_TYPE_VIDEO:
			addVideoStream(inputStream.getIndex(), inputStream.getId(), format.establishOutputCodecId(inputID),
					inputCoder.getFrameRate(), inputCoder.getWidth(), inputCoder.getHeight());
			break;
		default:
			break;
		}
	...
	public void open() {
		...
		if (getContainer().open(getUrl(), IContainer.Type.WRITE, mContainerFormat, true, false) < 0)
	...
	public void close() {
		...
		if ((rv = getContainer().writeTrailer()) < 0)
			throw new RuntimeException("error " + IError.make(rv) + ", failed to write trailer to " + getUrl());
		...
		if (getShouldCloseContainer()) {
			if ((rv = getContainer().close()) < 0)
				throw new RuntimeException("error " + IError.make(rv) + ", failed close IContainer " + getContainer()
						+ " for " + getUrl());
	...
	/**
	 * Get the default time base we'll use on our encoders if one is not
	...
	public void onVideoPicture(IVideoPictureEvent event) {
		if (event.getImage() != null)
			encodeVideo(event.getStreamIndex(), event.getImage(), event.getTimeStamp(event.getTimeUnit()),
					event.getTimeUnit());
		else
