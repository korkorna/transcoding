...
public class VideoConverterNotRunning extends AMediaCoderMixin implements IMediaWriter {
	...
	public VideoConverterNotRunning(String transcodeFile, IMediaReader reader, OutputFormat outputFormat) {
		this(transcodeFile, reader.getContainer(), outputFormat);
	...
	public VideoConverterNotRunning(String url, IContainer inputContainer, OutputFormat outputFormat) {
	    ...
	    // resize
	    resizeWidth = outputFormat.getWidth();
	    resizeHeight = outputFormat.getHeight();
	    videoBitRateInKilobytes = outputFormat.getBitrate();
	    videoCodec = CodecValueConverter.fromDomain(outputFormat.getVideoCodec());
	    audioCodec = CodecValueConverter.fromDomain(outputFormat.getAudioCodec());
	...
	private IVideoPicture makeResizedPicture(IVideoPicture picture) {
		...
		if (videoResampler == null) {
			videoResampler = IVideoResampler.make(this.resizeWidth,
					this.resizeHeight, picture.getPixelType(),
	...
	private boolean addStreamFromContainer(int inputStreamIndex) {
		...
		ICodec.ID codec = null;
		
		switch (inputType) {
		case CODEC_TYPE_AUDIO:
			codec = audioCodec == null ? format.establishOutputCodecId(inputID) : audioCodec;
					
			addAudioStream(inputStream.getIndex(), inputStream.getId(), codec,
					inputCoder.getChannels(), inputCoder.getSampleRate());
			break;
		case CODEC_TYPE_VIDEO:
			int width = hasResizeWidth() ? resizeWidth : inputCoder.getWidth();
			int height = hasResizeHeight() ? resizeHeight : inputCoder.getHeight();
			codec = videoCodec == null ? format.establishOutputCodecId(inputID) : videoCodec;
					
			addVideoStream(inputStream.getIndex(), inputStream.getId(), codec,
					inputCoder.getFrameRate(), width, height);// outputFormat.getWidth(),
															// outputFormat.getHeight());
			break;
		default:
			break;
		}
